...
import GoogleSignIn // NEW: For Google Sign-In
import FirebaseCore
...
final class AuthenticationViewModel: ObservableObject {
    ...
    func signInGoogle() async throws {
        // NEW: Use GoogleSignIn pod, replacing SignInGoogleHelper
        guard let clientID = FirebaseApp.app()?.options.clientID else {
            throw NSError(domain: "GoogleSignInError", code: -1, userInfo: [NSLocalizedDescriptionKey: "Google client ID not found"])
        }
        ...
        guard let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
            ...
            throw NSError(domain: "GoogleSignInError", code: -2, userInfo: [NSLocalizedDescriptionKey: "No root view controller found"])
        }
        
        // Perform Google Sign-In
        let gidSignInResult = try await GIDSignIn.sharedInstance.signIn(withPresenting: rootViewController)
        guard let idToken = gidSignInResult.user.idToken?.tokenString else {
            throw NSError(domain: "GoogleSignInError", code: -3, userInfo: [NSLocalizedDescriptionKey: "Google ID token not found"])
        }
    ...
    // RESTORED: Original SignInWithAppleHelper from your code
