...
final class UserManager {
    ...
    func getUser(userID: String, forceServer: Bool = false) async throws -> DBUser {
        ...
        let documentSnapshot = try await userDocument(userId: userID).getDocument(source: forceServer ? .server : .cache)
    ...
    // CHANGE: Add function to update user profile
    func updateUser(
        userId: String,
        firstName: String?,
        lastName: String?,
        ...
        var data: [String: Any] = [:]
        if let firstName = firstName { data[DBUser.CodingKeys.firstName.rawValue] = firstName }
        if let lastName = lastName { data[DBUser.CodingKeys.lastName.rawValue] = lastName }
        if let phone = phone { data[DBUser.CodingKeys.phone.rawValue] = phone }
        if let address = address { data[DBUser.CodingKeys.address.rawValue] = address }
        if let companyName = companyName { data[DBUser.CodingKeys.companyName.rawValue] = companyName }
        if let profession = profession { data[DBUser.CodingKeys.profession.rawValue] = profession }
        if let photoUrl = photoUrl { data[DBUser.CodingKeys.photoUrl.rawValue] = photoUrl }
        
        print("Updating Firestore with data: \(data)")
        try await userDocument(userId: userId).updateData(data)
